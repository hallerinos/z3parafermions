import ITensorMPS: DiskVector, OneITensor, disk
import Base.copy
"""
A DiskProjMPO1 computes and stores the projection of an
...
"""
mutable struct DiskProjMPO1 <: AbstractProjMPO
  lpos::Int
  rpos::Int
  nsite::Int
  H::MPO
...
function set_nsite!(P::DiskProjMPO1, nsite)
  P.nsite = nsite
  return P
end

function DiskProjMPO1(H::MPO)
  return new(
    0,
    length(H) + 1,
    1,
...
function disk(pm::ProjMPO1; kwargs...)
  return DiskProjMPO1(
    pm.lpos,
    pm.rpos,
    pm.nsite,
...
disk(pm::DiskProjMPO1; kwargs...) = pm

# Special overload of ITensorMPS.lproj which uses the cached
# version of the left projected MPO, and if the
# cache doesn't exist it loads it from disk.
function ITensorMPS.lproj(P::DiskProjMPO1)::Union{ITensor,OneITensor}
...
# Special overload of ITensorMPS.rproj which uses the cached
# version of the right projected MPO, and if the
# cache doesn't exist it loads it from disk.
function ITensorMPS.rproj(P::DiskProjMPO1)::Union{ITensor,OneITensor}
...
function makeL!(P::DiskProjMPO1, psi::MPS, k::Int)
  L = _makeL!(P, psi, k)
  if !isnothing(L)
    # Cache the result
    P.Lcache = L
...
function makeR!(P::DiskProjMPO1, psi::MPS, k::Int)
  R = _makeR!(P, psi, k)
  if !isnothing(R)
    # Cache the result
    P.Rcache = R
