...
import ITensorMPS.orthocenter
import ITensorMPS.eigsolve
include("projmpo0.jl")
"""
    dmrg0(H::MPO,psi0::MPS;kwargs...)
    dmrg0(H::MPO,psi0::MPS,sweeps::Sweeps;kwargs...)
...
"""
function dmrg0(H::MPO, psi0::MPS, sweeps::Sweeps; kwargs...)
    ...
    PH = ProjMPO0(H)
    return dmrg0(PH, psi0, sweeps; kwargs...)
end

"""
    dmrg0(Hs::Vector{MPO},psi0::MPS;kwargs...)
    dmrg0(Hs::Vector{MPO},psi0::MPS,sweeps::Sweeps;kwargs...)
...
Optional keyword arguments:
  ...
  - `write_when_maxdim_exceeds::Int` - when the allowed maxdim exceeds this
     value, begin saving tensors to disk to free memory in large calculations
"""
function dmrg0(Hs::Vector{MPO}, psi0::MPS, sweeps::Sweeps; kwargs...)
    for H in Hs
        check_hascommoninds(siteinds, H, psi0)
        check_hascommoninds(siteinds, H, psi0')
    end
    ...
    PHS = ProjMPOSum(Hs)
    return dmrg0(PHS, psi0, sweeps; kwargs...)
end

"""
    dmrg0(H::MPO,Ms::Vector{MPS},psi0::MPS;kwargs...)
    dmrg0(H::MPO,Ms::Vector{MPS},psi0::MPS,sweeps::Sweeps;kwargs...)
...
"""
function dmrg0(H::MPO, Ms::Vector{MPS}, psi0::MPS, sweeps::Sweeps; kwargs...)
    check_hascommoninds(siteinds, H, psi0)
    check_hascommoninds(siteinds, H, psi0')
    for M in Ms
    ...
    return dmrg0(PMM, psi0, sweeps; kwargs...)
end

function dmrg0(PH, psi0::MPS, sweeps::Sweeps; kwargs...)
    if length(psi0) == 1
        error(
    ...
    for sw in 1:nsweep(sweeps)
        sw_time = @elapsed begin
            ...
            for (b, ha) in sweepnext(N)
                ...
                if ha==1
                    # @show PH.lpos, PH.rpos, b
                    rind = commoninds(psi[b], psi[b+1])
                    cind = commoninds(PH.H[b], psi[b])
                    lind = noncommoninds(psi[b], unioninds(rind, cind))
                    # @show lind, cind, rind

                    # update phi*R[b]*R[b+1]
                    phi, psi[b] = factorize(psi[b], lind, ortho="right")
                    # remove phi from projected Ham
                    PH.LR[b] = ((PH.LR[b+1]*psi[b])*PH.H[b])*dag(psi[b])'
                    vals, vecs = eigsolve(
                        PH,
                        phi,
                        1,
                    ...
                    # @show PH.lpos, PH.rpos, b
                    lind = commoninds(psi[b], psi[b+1])
                    cind = commoninds(PH.H[b+1], psi[b+1])
                    rind = noncommoninds(psi[b+1], unioninds(lind, cind))
                    # @show lind, cind, rind

                    # update L[b]*L[b+1]*phi
                    psi[b+1], phi = factorize(psi[b+1], unioninds(lind, cind), ortho="left")
                    # remove phi from projected Ham
                    # @show inds(PH.LR[b-1])
                    # @show inds(PH.LR[b])
                    # @show inds(PH.LR[b+1])
                    # @show inds(psi[b+1])
                    # @show inds(phi)
                    PH.LR[b+1] = ((PH.LR[b]*psi[b+1])*PH.H[b+1])*dag(psi[b+1]')
                    # @show inds(PH.LR[b]), inds(PH.LR[b+1])
                    # @show energy
                    # @show lproj(PH)*phi*conj(phi')*rproj(PH)
                    vals, vecs = eigsolve(
                        PH,
                        phi,
                        1,
...
function dmrg0(x1, x2, psi0::MPS; kwargs...)
    return dmrg0(x1, x2, psi0, _dmrg_sweeps(; kwargs...); kwargs...)
end

function dmrg0(x1, psi0::MPS; kwargs...)
    return dmrg0(x1, psi0, _dmrg_sweeps(; kwargs...); kwargs...)
end
